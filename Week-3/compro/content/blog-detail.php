...
// $id = isset($_GET['id']) ? $_GET['id'] : '';
    $id = $_GET['id'] ?? '';
    $blogDetails = mysqli_query($koneksi, "SELECT categories.name as category_name, blogs.* FROM blogs JOIN categories ON categories.id = blogs.category_id WHERE blogs.id = '$id' ORDER BY blogs.id DESC");
    // All (Data lebih dari satu)
    $rowBlogDetail = mysqli_fetch_assoc($blogDetails);

    $recentBlogs = mysqli_query($koneksi, "SELECT categories.name as category_name, blogs.* FROM blogs JOIN categories ON categories.id = blogs.category_id ORDER BY blogs.id DESC LIMIT 5");
    // All (Data lebih dari satu)
